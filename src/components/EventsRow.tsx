  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-5 pt-6 sm:px-8">
      <div className="flex items-baseline justify-between gap-3">
        ...
        {!hasReal && <span className="caps text-[var(--ink-subtle)]">{inhouse.note}</span>}
      </div>
      ...
      {hasReal ? (
        ...
        <div className="no-scrollbar -mx-5 mt-3 flex gap-3 overflow-x-auto px-5 sm:mx-0 sm:px-0">
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className="flex h-[110px] w-[260px] shrink-0 items-center justify-center rounded-xl border border-dashed border-[var(--line)] sm:w-[300px]"
            >
              <span className="caps text-[var(--ink-faint)]">EVENT SLOT {String(i + 1).padStart(2, '0')}</span>
            </div>
          ))}
        </div>
      )}
